...
from aesthetics_model.model.mock_model import MockModel
from aesthetics_model.model.model import AestheticModel

AestheticsModelConfig.print_config()
log = logging.Logger("App-Logger")

model = MockModel() if AestheticsModelConfig.mockBackend else AestheticModel()

app = BaseApi(is_ready=model.is_ready, config=AestheticsModelConfig)
...
@app.post(
    ...
    response_model=List[AestheticsModelResponse],
    tags=[model_route_tag],
)
async def inference(images: List[UploadFile]) -> List[AestheticsModelResponse]:
